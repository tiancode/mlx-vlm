...
Uses local tokenizer files and the selected HTTP service; loads no model weights.
"""
import argparse
import json
import os
...
def remaining_context_budget(health, prompt_tokens):
    """Use the serving context limit, not a benchmark-specific constant."""
    limit = health.get("effective_context_limit")
    if not isinstance(limit, int) or limit <= 0:
        raise ValueError("Budget checks require a positive effective_context_limit")
    if not isinstance(prompt_tokens, int) or not 0 <= prompt_tokens < limit:
        raise ValueError("Prompt must leave a positive generation budget")
...
def main():
    ...
    if not os.environ.get("API_KEY"):
        parser.error("Set API_KEY to the running service credential")
    ...
    def request(label, text, max_output_tokens=None, expected_status=200):
        payload = {"model": args.model, "messages": [{"role": "user", "content": text}],
        ...
        if args.endpoint == "responses":
            ...
            payload["store"] = False
        req = urllib.request.Request(base_url + "/v1/" + args.endpoint,
        ...
        baseline = service_state("/metrics") if args.disconnect_after else None
        if baseline:
    ...
    if args.reuse_prefix:
        if args.disconnect_after:
            # More than eight uncached tokens force prefill after checkpoint restore.
            # The server applies its configured context budget to the whole request.
    ...
    if cached < max(1, args.tokens - 4096):
    ...
    if args.check_budget:
        ...
        exact_budget = remaining_context_budget(health, prompt_tokens)
        results["checked_context_limit"] = health["effective_context_limit"]
