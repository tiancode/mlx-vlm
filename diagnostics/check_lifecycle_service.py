"""Check completion, disconnects and cancellation through an idle local proxy."""
import argparse
import json
import os
...
def main():
    ...
    def payload(text, stream=False):
        return {"model": args.model, "input": text, "stream": stream,
    ...
    with httpx.Client(base_url=args.base_url.rstrip("/"), headers=headers, timeout=120) as client:
        ...
        state = metrics.json()
        if state["summary"]["in_flight"] or state["server"]["request_queue_depth"]:
            raise RuntimeError("Service is busy; lifecycle checks aborted")
        ...
        with client.stream("POST", "/v1/responses", json=payload(text, stream=True)) as response:
            ...
            for line in response.iter_lines():
                if "response.created" in line:
                    time.sleep(3)  # Let prefill begin before disconnecting.
                    break
        since = time.monotonic()
